...
helm、bare manifest）和 namespace 都不一样。k8s-mcp 走"两层发现"协议：
...
2. **Agent 挑一个 URL，传给 Prometheus 工具** —— `prometheus_query(promql,
   prometheus_url=<那个 URL>)` / `prometheus_query_range(...)` /
   `pod_metrics(..., prometheus_url=<那个 URL>)`。
...
> 拿到 `default/monitor-kube-prometheus-st-prometheus` 那行的 URL →
