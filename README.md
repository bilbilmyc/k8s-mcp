...
- `prometheus_query(promql, time?)` — Prometheus 即时 PromQL 查询（**不是** metrics-server，可以查任意 Prometheus 已抓取的指标）
- `prometheus_query_range(promql, start, end, step="30s")` — Prometheus 时间序列范围查询
- `pod_metrics(pod_name, namespace, metric="cpu|memory|network_rx|network_tx|fs_reads|fs_writes", range="5m")` — 用 cAdvisor 默认指标名查某个 Pod 的 CPU / 内存 / 网络 / 磁盘读写速率
...
**Prometheus 怎么找**：自动探测 `monitoring` / `prometheus` /
`kube-prometheus` / `observability` 几个 namespace 里名为
`prometheus` / `prometheus-operated` /
`kube-prometheus-stack-prometheus` / `prometheus-server` 的 Service。
找不到时返回"问用户"的提示，让用户给 URL，然后通过
`K8S_MCP_PROMETHEUS_URL` 注入。
